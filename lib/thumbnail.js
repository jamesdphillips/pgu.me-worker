...
module.exports = function (path, styles, cb) {
  var results = [];
  var completed = 0;
  var complete = function (err) {
    if (err) { completed = -1; cb(err); }
    else { completed++; }
    if (completed == Object.keys(styles).length) { cb(undefined, results); }
  }
...
var getImageSize = function (file, cb) {
  gm(file).size(function(err, value) {
    if (!err && value) {
      ...
      cb(new Error('Unable to get the dimensions.'));
    }
  });
}
...
var resizeAbsolute = function (file, style, cb) {
  getImageSize(file, function (err, size) {
    if (!err) {
      ...
      var f = gm(file).quality(style.quality).setFormat(style.format).crop(rect.width, rect.height, rect.x, rect.y).resize(style.width, style.height);
      cb(undefined, f);
...
var shrinkToDimensions = function (file, style, cb) {
  getImageSize(file, function (err, size) {
    if (!err && size) {
      ...
      var f = gm(file).quality(style.quality).setFormat(style.format).resize(style.width, style.height);
      cb(undefined, f);
    }
    else { cb(err); }
  });
